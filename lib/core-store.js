...
exports.NewKeyValueClient = function (reqEndpoint, enableLogging) {
    ...
    let kvc = {
        ...
        Observe: function (dataSourceID, timeOut = 60, contentFormat = "JSON") {
            let path = "/kv/" + dataSourceID;
            return _observe(this.zestClient,this.zestEndpoint, path, timeOut = 60, contentFormat);
        },
        ...
        RegisterDatasource: function (DataSourceMetadata) {
            return _registerDatasource(this.zestClient,this.zestEndpoint,DataSourceMetadata);
        },
...
exports.NewTimeSeriesClient = function (reqEndpoint, enableLogging) {
    let tsc = {
        ...
        Observe: function (dataSourceID, timeOut) {
            let path = "/ts/" + dataSourceID;
            return _observe(this.zestClient,this.zestEndpoint, path, timeOut = 60, 'JSON');
        },
        StopObserving: function (dataSourceID) {
            let path = "/ts/" + dataSourceID;
            this.zestClient.StopObserving(path);
...
let _registerDatasource = function (zestClient, zestEndpoint, DataSourceMetadata) {
    return new Promise((resolve, reject) => {
...
let _observe = function (zestClient, zestEndpoint, path, timeOut = 60, contentFormat = "JSON") {
    return new Promise((resolve, reject) => {
        ...
        .then((token)=>{
            ...
            .catch((err)=>{
                reject("OBSERVE ERROR: for path " + tokenPath + " " +err);
            });

        })
        .catch((err)=>{
            reject("OBSERVE TOKEN ERROR: for path " + tokenPath + " " + err);
        });
