...
exports.NewCoreStoreClient = function (DataSourceMetadataString, enableLogging) {

    return new Promise((resolve,reject)=>{
        DataSourceMetadataToHypercat(DataSourceMetadataString)
        .then((Metadata)=>{
            switch(Metadata.DataSourceMetadata.StoreType) {
                case 'ts':
                    resolve(NewTimeSeriesClient(DataSourceMetadata.DataSourceURL, enableLogging));
                    break;
                case 'tsblob':
                    resolve(NewTimeSeriesBlobClient(DataSourceMetadata.DataSourceURL, enableLogging));
                    break;
                case 'kv':
                    resolve(NewKeyValueClient(DataSourceMetadata.DataSourceURL, enableLogging));
                    break;
                default:
                    reject("Unsupported StoreType ", Metadata.DataSourceMetadata.StoreType)
            }
        })
    });
}
