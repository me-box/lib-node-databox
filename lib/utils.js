...
exports.makeStoreRequest = function() {
	return (options, caveatObj) => {
		...
		if(caveatObj !== null && typeof caveatObj === 'object') {
			routeHash += JSON.stringify(caveatObj)
		}

		return new Promise((resolve, reject) => {
			Promise.resolve().then(() => {
				if (!(routeHash in requestTokenCache))
					return exports.requestToken(route.target, route.path, route.method, caveatObj);
				return requestTokenCache[routeHash];
			}).then((token) => {
				...
				delete requestTokenCache[routeHash]
				reject(err)
			});
		});
	};
...
exports.requestToken = function(hostname, endpoint, method, caveatObj) {

	return new Promise((resolve, reject) => {

		let caveatJson = ""
		if(caveatObj !== null && typeof caveatObj === 'object') {
			caveatJson = JSON.stringify(caveatObj);
		}

		let key = crypto.createHash('md5').update(hostname+endpoint+method+caveatJson).digest('hex');
		...
		if(key in requestTokenCache) {
			...
			if(caveatObj !== null && typeof caveatObj === 'object') {
				req.caveats = [caveatObj];
			}
			exports.makeZestArbiterPOSTRequest('/token', req)
			.then((res)=>{
